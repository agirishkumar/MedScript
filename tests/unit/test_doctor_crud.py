...
def test_update_doctor_duplicate_email(mock_db_session: Session):
    """
    Tests that updating a doctor's email to an existing email raises an HTTPException.
    """
    doctor_data1 = DoctorCreate(
        FirstName="John", LastName="Doe", Specialty="Neurologist",
        LicenseNumber="12345", ContactNumber="9876545678", Email="test@example.com"
    )
    create_doctor(mock_db_session, doctor_data1)

    doctor_data2 = DoctorCreate(
        FirstName="Jane", LastName="Smith", Specialty="Cardiologist",
        LicenseNumber="54321", ContactNumber="8765432109", Email="jane@example.com"
    )
    new_doctor = create_doctor(mock_db_session, doctor_data2)

    updated_data = DoctorUpdate(Email="test@example.com")
    with pytest.raises(HTTPException) as excinfo:
        update_doctor(mock_db_session, new_doctor.DoctorID, updated_data)
    assert excinfo.value.status_code == 400
    assert "Email already registered" in str(excinfo.value.detail)
