...
def create_patient_visit(visit: PatientVisitCreate, db: Session = Depends(get_db)):
    ...
    """
    return patient_visit_crud.create_patient_visit(db=db, visit=visit)
...
def read_patient_visits(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    ...
    """
    return patient_visit_crud.get_patient_visits(db, skip=skip, limit=limit)
...
def update_patient_visit(visit_id: int, visit: PatientVisitUpdate, db: Session = Depends(get_db)):
    ...
    """
    db_visit = patient_visit_crud.update_patient_visit(db=db, visit_id=visit_id, visit=visit)
    if db_visit is None:
        raise HTTPException(status_code=404, detail="Patient visit not found")
    return db_visit
